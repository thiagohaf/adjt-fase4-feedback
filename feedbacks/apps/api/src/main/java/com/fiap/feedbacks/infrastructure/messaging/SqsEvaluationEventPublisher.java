...
/**
 * Adapter SQS (%aws) — preparação (AD-5/AD-7). Fila ainda não provisionada: no-op seguro com log.
 * Ativo quando o build profile é {@code aws}.
 */
@ApplicationScoped
@IfBuildProfile("aws")
public class SqsEvaluationEventPublisher implements EvaluationEventPublisher {
    ...
    @Inject
    public SqsEvaluationEventPublisher(ObjectMapper objectMapper) {
    ...
    public void publish(AvaliacaoAlertaEvent event) {
        ...
        LOG.infof("SqsEvaluationEventPublisher (no-op — fila não provisionada): payload=%s", payload);
    }
