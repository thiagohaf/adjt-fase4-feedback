...
> Deriva de `openspec/modules/01-autenticacao-e-papeis/specs.md` (FR-2, SPEC-2.1–2.11) e da
> matriz de autorização do proposal.md §3.3 / AD-8. A lógica de negócio de
> Curso/Aula/Inscrição/Avaliação pertence aos módulos 02+; neste módulo os recursos existem como
> **stubs mínimos** com o `@RolesAllowed` da matriz, o suficiente para provar a autorização
> server-side (mesma abordagem do branch de referência Spring). A regra fina de SPEC-2.11 usa a
> porta `CurrentUserProvider` sobre um read model mínimo de Avaliação.
