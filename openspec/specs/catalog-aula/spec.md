...
Criação e consulta de Aula vinculada a Curso na API Quarkus (`feedbacks/apps/api/`):
campo público **`nome`** (não `titulo`), FK para Curso, códigos `CURSO_NOT_FOUND` /
`AULA_NOT_FOUND` / `VALIDATION_ERROR`, e autorização com papéis do módulo 01. Derivado
da change `catalogo-curso-aula-quarkus` (módulo 02, FR-3/FR-4).
