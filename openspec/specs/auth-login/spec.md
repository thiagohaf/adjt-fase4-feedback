...
Autenticação por email/senha com emissão de JWT HS256 (claims `sub`, `role`, `iat`, `exp`),
validação de payload, armazenamento BCrypt e logs sem exposição de senha. Implementado na
API Quarkus em `feedbacks/apps/api/` (change `autenticacao-e-papeis-quarkus`).
